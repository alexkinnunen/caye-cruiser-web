...
const RandomUnderline = ({ children, color = "#ffffff", className = "" }: RandomUnderlineProps) => { // Lime/Primary color
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const leaveTween = leaveTweenRef.current;
  ...
  return (
    <span ref={containerRef} className={`relative inline-block ${className}`}>
      ...
      <div
        ref={boxRef}
        className="absolute left-0 right-0 bottom-0 h-4 md:h-6 pointer-events-none"
        style={{ color, top: '1.2em' }}
      />
    </span>
  );
};
